...
describe('framesReducer: CHANGE_ACTIVE_FRAME', () => {
  it('should set the given frame as active', () => {
    ...
    const nextState = reducer(
      state,
      actions.changeActiveFrame(frameIndex)
    )
...
describe('framesReducer: DUPLICATE_FRAME', () => {
  ...
  it('creates a new frame with the same grid colors', () => {
    expect(
      nextState.getIn(['list', frameId + 1, 'grid']).toJS()
    ).toEqual(firstGridMock)
  })
  ...
  it('grids after id + 1 are moved one index to the right', () => {
    expect(
      nextState.getIn(['list', frameId + 2, 'grid']).toJS()
    ).toEqual(secondGridMock)
  })
  ...
  describe('changing columns', () => {
    ...
    describe('number of columns increments', () => {
      beforeEach(() => {
        ...
        nextState = reducer(
          state,
          actions.changeDimensions(gridProperty, 1)
        )
      })
      ...
      it('columns value is incremented', () => {
        expect(nextState.get('columns')).toEqual(
          state.get('columns') + 1
        )
      })
    })
...
describe('framesReducer: CHANGE_HOVERED_CELL', () => {
  it('change the position of the hovered cell', () => {
    ...
    const nextState = reducer(
      state,
      actions.changeHoveredCell({ x: 2, y: 3 })
    )
    expect(nextState.get('hoveredIndex').get('x')).toEqual(
      exepectedPosition.x
    )
    expect(nextState.get('hoveredIndex').get('y')).toEqual(
      exepectedPosition.y
    )
  })
})
