...
describe('reducer: SET_INITIAL_STATE', () => {
  it('palette grid should be initialized', () => {
    ...
    expect(nextGrid.getIn([23, 'color'])).toEqual(
      'rgba(255, 255, 255, 1)'
    )
    expect(nextGrid.getIn([24, 'color'])).toEqual(
      'rgba(56, 53, 53, 1)'
    )
    expect(nextGrid.getIn([29, 'color'])).toEqual(
      'rgba(56, 53, 53, 1)'
    )
  })
...
describe('reducer: NEW_PROJECT', () => {
  it('palette grid should be initialized', () => {
    ...
    expect(nextGrid.getIn([10, 'color'])).toEqual(
      'rgba(76, 175, 80, 1)'
    )
    expect(nextGrid.getIn([20, 'color'])).toEqual(
      'rgba(158, 158, 158, 1)'
    )
  })
...
describe('paletteReducer: APPLY_EYEDROPPER', () => {
  ...
  describe('cell color is not included in palette', () => {
    ...
    it('should update the last cell of grid', () => {
      expect(
        nextState.getIn(['grid', grid.size - 1, 'color'])
      ).toEqual(CELL_COLOR)
    })
  ...
  describe('cell color is included in palette', () => {
    ...
    it('should not update the grid', () => {
      expect(nextState.get('grid').toJS()).toEqual(
        state.get('grid').toJS()
      )
    })
...
describe('reducer: SET_CUSTOM_COLOR', () => {
  ...
  describe('palette color is not selected', () => {
    beforeEach(() => {
      ...
      nextState = reducer(
        state,
        actions.setCustomColor(rgbColorValues)
      )
    })
  ...
  describe('palette color is selected', () => {
    it('should set the custom color in selected palette cell', () => {
      ...
      expect(nextState.getIn(['grid', position, 'color'])).toEqual(
        customColor
      )
    })
  })
