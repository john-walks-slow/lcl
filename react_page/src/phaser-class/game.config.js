export default function configureScene() {
  ...
  c.ZOOM_LEVEL = Math.round(Math.max(Math.min(c.WINDOW_W / 13, c.WINDOW_H / 14 / 44 * 37),) / c.PLAYER_TARGET_W);
  c.ZOOM_OUT_LEVEL = (0.3 * c.ZOOM_LEVEL);
  c.OBJECT_W = { XXL: c.PLAYER_TARGET_H * 3, XL: c.PLAYER_TARGET_H * 2, L: c.PLAYER_TARGET_H * 1.5, M: c.PLAYER_TARGET_H * 1, S: c.PLAYER_TARGET_H * 0.7, XS: c.PLAYER_TARGET_H * 0.5 };
  c.TIME_DELAY = 60 * 60 * 1000;
  c.RANDOM_ZONE_W = c.OBJECT_W.L;
  c.DAY_OFFSET = c.OBJECT_W.M;
  c.DENSITY_OFFSET = c.OBJECT_W.L;
  c.ACTIVITY_OFFSET = 1;
  c.MOVE_SPEED = (c.PLAYER_TARGET_W * 1.5);
