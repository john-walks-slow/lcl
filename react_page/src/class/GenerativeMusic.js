...
class GenerativeMusic {
  constructor() {
    ...
    MS.Scale.prototype.getNote = function(i) {
      const degree = i.mod(this.notes().length)
      const octave = (i - degree) / this.notes().length + 5
      // console.log(degree);
    ...
    const generateChord = () => {
      ...
      const rootPadSynth = new Tone.PolySynth(Tone.Synth, {
    ...
    switch (o.sound) {
      case 'pad':
        ...
        o.loop = new Tone.Sequence((time, note) => {
          ...
          const notes = [note, note + 2, note + 4].map(n =>
  ...
  startBgm() {
    ...
    if (Tone.context.state == 'running') {
      ...
      const clickListener = document.addEventListener('click', start)
      const touchListener = document.addEventListener(
        'touchend',
        start
      )
      const keyListener = document.addEventListener('keydown', start)
    }
  }
}
