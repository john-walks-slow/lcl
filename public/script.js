...
function createTestObject(object) {
  ...
  // let rad = object.seed[0] * (Math.PI / 180);
  // let distance = object.seed[1] / 100 * RANDOM_ZONE_W;
  // let oY = Math.cos(rad) * distance;
  // let oX = Math.sin(rad) * distance;
  // let isOver = false;
  // for (let index = 0; index < objectList.length; index++) {
  // 	const oo = objectList[index];
  // 	let rad = oo.seed[0] * (Math.PI / 180);
  //     let distance = oo.seed[1] / 100 * RANDOM_ZONE_W;
  //     let ooY = Math.cos(rad) * distance;
  //     let ooX = Math.sin(rad) * distance;
  //     if (Math.abs(ooY-oY)*2<(OBJECT_W[object.size]+OBJECT_W[oo.size] || Math.abs(ooX-oX)*2<(OBJECT_W[object.size]+OBJECT_W[oo.size]))){
  // 		isOver = true;
  //     }
  // 	console.log(Math.abs(ooY-oY)*2,OBJECT_W[object.size],OBJECT_W[oo.size]);
  // };
  // // console.log(isOver);
  // if (isOver) {object = createTestObject(object)};
  return object;
}
function createObject(object) {
  object.id = (Number(this.objectList[this.objectList.length - 1].id) + 1).toString();
  object.seed = [Math.random() * 360, Math.random() * 100];
  console.log(object);
  return object;
}
...
class LoadingScene extends Phaser.Scene {
  ...
  create() {
    ...
    app.service('objects').find().then(({ data }) => {
      ...
      //   }))
      // });
      objectList.forEach((o) => { this.load.image("object" + o._id, 'assets/objects/' + o._id + '.png') });
      // console.log(objectList);
...
class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    let offset;
    let dateOffset = (PLAYER_TARGET_H + OBJECT_W.XL) / 2 + 5;
    this.objectList.forEach((o, i) => {
      if (timestamp - o.birthday < TIME_DELAY) { return; }
      dateOffset += Math.min(1, (30 * 24 * 60 * 60)/(previousDate - o.birthday)) * 5 * PLAYER_TARGET_W;
      offset = (DENSITY_OFFSET * i) **0.6;
    ...
    let updateObjects = (previousZone, currentZone) => {
      ...
      createZones.forEach((zone) => {
        ...
        os.forEach((o) => {
          ...
          o.instance.setDisplaySize(OBJECT_W[o.size], OBJECT_W[o.size]);
