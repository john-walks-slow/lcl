...
const Page = ({ dispatch, isShown }) => {
  ...
  const framesList = useSelector(state =>
    state.present.get('frames').get('list')
  )
    ...
    <div className={'page__container' + (show ? ' show' : '')}>
      ...
      {submitted || errorData ? (
        <div className={'page__submitted' + ' show'}>
          {errorData
            ? '哎呀！出了一点问题。' + errorData.toString()
            : '创建成功了！一小时之后，它会出现在世界里'}
          <br />
          {rewardColor ? (
            <div
              className="page__div-color"
              style={{ backgroundColor: rewardColor + '10' }}
            >
              <span
        ...
        <form className={'page__form' + (submitted ? ' hide' : '')}>
          <h1>新东西</h1>
          <label className={'page__label'}>形象</label>
          ...
          >
            ...
            <sub>
              {'肥料数量：' +
                fats +
                (fatConsumption == 0 ? '' : `(-${fatConsumption})`)}
            </sub>
            <select
              value={size}
              onChange={event => {
